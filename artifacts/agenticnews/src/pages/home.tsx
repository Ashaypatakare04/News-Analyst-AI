...
import {
  Loader2, Search, Brain, TrendingUp, Sparkles, ShieldCheck,
  Activity, ArrowRight, ArrowRightCircle, Zap, Target, BarChart3,
        ...
        <div className="flex items-center gap-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-center gap-6 text-muted-foreground">
            <span className="flex items-center gap-1.5">
              ...
              {newsData?.total || 0} articles indexed
            </span>
            <span className="flex items-center gap-1.5">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 lg:w-2/3 flex flex-col gap-8">
            ...
            <motion.div
              ...
              className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/5 via-card to-card overflow-hidden shadow-lg"
            >
              <div className="flex items-center gap-3 px-6 pt-5 pb-4 border-b border-border/40">
                ...
                <div>
                  <h3 className="font-bold text-foreground text-base">Today's Intelligence</h3>
                  <p className="text-xs text-muted-foreground">AI-generated signal from {intelligence?.sourcesAnalyzed ?? "—"} sources</p>
                </div>
                <Link href="/brief" className="ml-auto text-xs text-primary hover:underline flex items-center gap-1 font-medium">
              ...
              {intelligenceLoading ? (
                <div className="flex items-center justify-center py-10">
                  ...
                  <span className="ml-3 text-sm text-muted-foreground">Generating intelligence...</span>
                </div>
              ) : intelligence ? (
                <div className="grid sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-border/40">
                  <div className="p-5">
                    ...
                    <p className="text-lg font-bold text-foreground leading-snug">{intelligence.topSignal}</p>
                  </div>
                  ...
                  <div className="p-5 sm:col-span-1">
                    ...
                    <p className="text-sm text-foreground/85 leading-relaxed">{intelligence.keyInsight}</p>
                  </div>
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-end gap-2 mb-2">
                      <span className="text-3xl font-bold text-foreground">{intelligence.confidence}%</span>
                    </div>
                    <div className="w-full bg-secondary/50 rounded-full h-1.5">
                      <div
                        className="h-1.5 rounded-full bg-gradient-to-r from-primary to-primary/70 transition-all duration-1000"
                        style={{ width: `${intelligence.confidence}%` }}
                      />
                    </div>
                    <p className="text-xs text-muted-foreground mt-1.5">{intelligence.sourcesAnalyzed} sources analyzed</p>
                  </div>
                </div>
              ) : (
                <div className="py-8 text-center text-muted-foreground text-sm">
                  Intelligence unavailable — fetch articles first
                </div>
              )}
            </motion.div>
          ...
          <div className="w-full lg:w-1/3 space-y-8 lg:sticky lg:top-[90px] h-fit self-start">
            ...
            {/* Daily Brief CTA */}
            <Link href="/brief" className="block bg-gradient-to-br from-amber-500/10 to-orange-500/5 border border-amber-500/20 rounded-3xl p-6 group hover:border-amber-500/40 transition-colors relative overflow-hidden">
              ...
              <h3 className="text-lg font-bold text-foreground mb-2 flex items-center gap-2">
                Daily Intelligence Brief <Sparkles className="w-4 h-4 text-amber-400" />
              </h3>
              <p className="text-sm text-muted-foreground mb-4">
                3 key events, 2 emerging signals, 1 strategic insight — curated by AI.
              </p>
              <div className="text-amber-400 font-medium text-sm flex items-center gap-1 group-hover:gap-2 transition-all">
                Read today's brief <ArrowRight className="w-4 h-4" />
              </div>
            </Link>
            ...
            <div className="bg-card border border-border/50 rounded-3xl p-6">
              ...
              {trendingLoading ? (
                <div className="flex items-center gap-3 py-4">
                  ...
                  <span className="text-sm text-muted-foreground">Analyzing topics...</span>
                </div>
              ) : trendingData?.topics?.length ? (
                <ul className="space-y-3">
                  {trendingData.topics.map((topic) => (
                    <li key={topic.rank} className="flex items-center gap-3 group cursor-pointer">
                      <span className="w-6 h-6 rounded-full bg-secondary text-muted-foreground flex items-center justify-center text-xs font-bold group-hover:bg-primary/20 group-hover:text-primary transition-colors shrink-0">
                        {topic.rank}
                      </span>
                      <span className="text-sm font-medium text-foreground/80 group-hover:text-primary transition-colors flex-1">
                        {topic.topic}
                      </span>
                ...
                <p className="text-sm text-muted-foreground py-2">Fetch articles to generate trending topics.</p>
              )}
            </div>
